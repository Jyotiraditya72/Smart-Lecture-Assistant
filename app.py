...
# Google Gemini API
genai.configure(api_key="AIzaSyAONJiWpWysAIJg8hzMzvK4fcZjXPUoN1w")
...
def generate_unique_mcqs(summary, num_questions=30, difficulty="medium"):
    try:
        model = genai.GenerativeModel('gemini-2.0-flash')
        prompt = f"""
        You are an advanced AI specializing in educational content generation. 
        Your task is to generate informative, challenging, and pedagogically sound multiple-choice questions (MCQs) based on the input text provided. 
        Ensure the questions assess key concepts, encourage critical thinking, and test deeper understanding—not just surface recall.
...
def index():
    if request.method == "POST":
        ...
        if not text or "Error" in text:
            return render_template("index.html", error="Failed to extract or transcribe content.")
...
def download_summary_txt():
    return send_file("uploads/summary.txt", as_attachment=True)
...
def download_mcqs_txt():
    return send_file("uploads/mcqs.txt", as_attachment=True)
